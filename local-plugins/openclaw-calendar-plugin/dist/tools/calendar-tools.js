...
const CALENDAR_CLI_CWD = process.env.CALENDAR_CLI_CWD || "C:\\Users\\chest\\Desktop\\calendar_service";
...
async function runCalendarCli(args) {
    try {
        const { stdout, stderr } = await execFileAsync(PYTHON_CMD, ["-m", "scripts.calendar_tool_cli", ...args], {
            cwd: CALENDAR_CLI_CWD,
            windowsHide: true,
        });
        const text = [stdout, stderr].filter(Boolean).join("\n").trim();
        return makeTextResult(text || "Command completed, but no output was returned.", {
            success: true,
            args,
            raw: text,
        });
...
export const calendarAddTool = {
    name: "calendar_add",
    label: "Calendar Add",
    description: "Add a calendar event for today, tomorrow, or a specified month/day.",
    parameters: Type.Object({
    ...
    async execute(_id, params) {
        ...
        if (params.date_type === "today") {
            ...
            if (params.month == null || params.day == null) {
                return makeTextResult("When date_type is 'date', both month and day are required.", {
                    success: false,
